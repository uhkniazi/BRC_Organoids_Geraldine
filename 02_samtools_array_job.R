...
q = paste0('select count(File.idSample) as files, Sample.idData, Sample.title, Sample.id as SampleID from File, Sample
           where (Sample.idData = 28 and File.idSample = Sample.id) group by File.idSample')
dfQuery = dbGetQuery(db, q)
dfQuery$title = gsub(" ", "", dfQuery$title, fixed = T)
dfQuery
# get the count of files
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 28) and (File.idSample = Sample.id) and (File.type = "bam")')
dfCounts = dbGetQuery(db, q)
head(dfCounts)
nrow(dfCounts)

# for each sample id, get the corresponding bam files
cvQueries = paste0('select File.*, Sample.title from File, Sample 
                   where (Sample.idData = 28 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) and 
                   (File.type = "bam")')
...
temp = lapply(cvQueries, function(x){
  ...
  return(data.frame(idSample=dfFiles$idSample[1], name=c(s4, s5), type=c('quality 10 sorted bam',
...
# dfNewData$group1 = 'Files aligned by the sequencing center'

...
## the genome used by the sequencing center has the chromosomes names without the word
## chr, hence just use chromosome number
...
q = paste0('select Sample.id as sid, Sample.group1, Sample.group2, Sample.group3, Sample.title, Sample.description, File.* from Sample, File
           where (Sample.idData = 28) AND (File.idSample = Sample.id AND File.type like "%_sort%")')
...
setwd(gcswd)
n = make.names(paste('CBamScaffold pre duplicate removal data id 28 geraldine rds'))
lAllBams$meta = dfSample
lAllBams$desc = paste('CBamScaffold object from bam files before duplicate removal for geraldine', date())
...
#                 comment='CBamScaffold object from bam files before duplicate removal in rna seq data for geraldine and mouse data')
...
setwd(gcswd)
n = make.names(paste('CBamScaffold after duplicate removal data id 28 geraldine rds'))
lAllBams$meta = dfSample
lAllBams$desc = paste('CBamScaffold object from bam files after duplicate removal for geraldine', date())
n2 = paste0('~/Data/MetaData/', n)
save(lAllBams, file=n2)
...
# dbListFields(db, 'MetaFile')
# df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/',
#                 comment='CBamScaffold object from bam files after duplicate removal in rna seq data for geraldine and mouse data')
# dbWriteTable(db, name = 'MetaFile', value=df, append=T, row.names=F)
# dbDisconnect(db)
...
dfSample.pre = lAllBams.pre$meta
lAllBams.pre$desc = NULL
lAllBams.pre$meta = NULL
names(lAllBams.pre) = dfSample.pre$title

iReadCount.pre = sapply(lAllBams.pre, f1)

mReadCount = rbind(iReadCount, iReadCount.pre)
rownames(mReadCount) = c('Post', 'Pre')

barplot(mReadCount, beside=T, las=2, main='No. of reads aligned', ylab = 'No. of Reads in Millions', cex.names =0.6,
        ylim=c(0, max(mReadCount)+10))
...
fGroups = cut(as.numeric(dfSample$group2), 3)
